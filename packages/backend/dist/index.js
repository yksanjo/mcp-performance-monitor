...
const agent_1 = require("@mcp-monitor/agent");
...
// Initialize monitor
const monitor = new agent_1.MCPMonitor();
...
app.get('/api/servers/:id', async (req, res) => {
    try {
        const servers = await monitor.getServers();
        const server = servers.find(s => s.id?.toString() === req.params.id || s.name === req.params.id);
        if (!server) {
...
app.get('/api/servers/:id/metrics', async (req, res) => {
    try {
        ...
        const metrics = await monitor.getServerMetrics(server.name, startDate && endDate ? { start: startDate, end: endDate } : undefined);
        ...
        const logs24h = recentLogs.filter(log => log.timestamp > twentyFourHoursAgo);
...
app.get('/api/metrics', async (req, res) => {
    try {
        ...
        const allMetrics = await monitor.getAllMetrics(startDate && endDate ? { start: startDate, end: endDate } : undefined);
...
app.get('/api/servers/compare', async (req, res) => {
    try {
        ...
        for (const serverId of serverIds) {
            const server = servers.find(s => s.id?.toString() === serverId || s.name === serverId);
            if (server) {
